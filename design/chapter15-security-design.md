...
│                        BFE 数据面                                │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────────────────┐ │
│  │ WAF / 黑名单 │  │ API-Key    │  │ 模型白名单 / 黑名单      │ │
│  │ IP 子网控制  │  │ 认证与配额  │  │ (来自 API-Key / Entity) │ │
│  └─────────────┘  └──────┬──────┘  └─────────────────────────┘ │
│                          ▼                                      │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────────────────┐ │
│  │ RPM 限流    │  │ TPM 限流    │  │ 并发限流                │ │
...
- 检测并拦截包含敏感关键词、Prompt 注入、越狱尝试的请求体；
...
上图展示了从网络层到应用层的逐层拦截顺序。WAF 与黑名单位于最前端，可过滤大量恶意流量；后续 API-Key、模型、配额、限流等机制进一步细粒度控制。
...
- BFE WAF 模块、IP 子网控制、模型黑白名单共同构成请求准入的安全屏障，建议按“WAF → IP 白名单 → API-Key 认证 → 模型黑白名单 → 配额/限流”顺序逐层拦截。
